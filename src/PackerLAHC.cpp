...
enum MoveType { MOVE_SWAP, MOVE_INSERT, MOVE_ROTATE, MOVE_REPAIR };
...
static void ShuffleGroups(std::vector<Packer::Item>& items, LCG& rng)
{
    ...
    while (i < (int)items.size())
    {
        ...
        while (i + g.count < (int)items.size()
               && items[i + g.count].itemTypeId == typeId)
            ++g.count;
        groups.push_back(g);
        i += g.count;
    }
...
Packer::Result Packer::PackAnnealed(int gridW, int gridH,
                                     const std::vector<Item>& items,
                                     TargetDim dim, int target,
                                     volatile long* abortFlag,
                                     const std::vector<Item>* seedOrder,
                                     std::vector<Item>* outBestOrder,
                                     int skipLAHCIfAreaBelow,
                                     const SearchParams* params,
                                     PackDiagnostics* outDiag)
{
    if (dim == TARGET_H)
        return PackAnnealedH(gridW, gridH, items, target, abortFlag,
                             seedOrder, outBestOrder,
                             skipLAHCIfAreaBelow, params, outDiag);
    ...
    Result r = PackAnnealedH(gridH, gridW, itemsT, target, abortFlag,
                             seedTPtr, outBestOrder,
                             skipLAHCIfAreaBelow, params, outDiag);
...
Packer::Result Packer::PackAnnealedH(int gridW, int gridH,
                                     const std::vector<Item>& items, int target,
                                     volatile long* abortFlag,
                                     const std::vector<Item>* seedOrder,
                                     std::vector<Item>* outBestOrder,
                                     int skipLAHCIfAreaBelow,
                                     ...
                                     PackDiagnostics* outDiag)
{
    ...
    int effGroupingWeight = (params && params->scoringGroupingWeight > 0) ? params->scoringGroupingWeight : Packer::DEFAULT_GROUPING_WEIGHT;
    int effFragWeight     = (params && params->scoringFragWeight     > 0) ? params->scoringFragWeight     : Packer::DEFAULT_FRAG_WEIGHT;
    ...
    int effSkylineWasteCoef = (params && params->skylineWasteCoef >= 1)
                              ? params->skylineWasteCoef
                              : Packer::DEFAULT_SKYLINE_WASTE_COEF;
    ...
    int effGroupingPower = (params && params->groupingPowerQuarters >= 1
                                   && params->groupingPowerQuarters <= 8)
    ...
    if (target > 1 && enablePreReservation)
    {
        for (int w = gridW; w >= MIN_RESERVE_W; --w)
        {
            ...
            int rx = gridW - w;
            MaxRectsPack(ctx, gridW, gridH, order, target,
                         abortFlag, rx, w);
            ++diagPackCalls;
    ...
    MaxRectsPack(ctx, gridW, gridH, order, target, abortFlag,
                 bestReserveX, bestReserveW, 0);  // BSSF
    ++diagPackCalls;
    if (abortFlag && *abortFlag != 0)
    {
        Result r;
        r.lerArea = 0; r.lerWidth = 0; r.lerHeight = 0;
        r.lerX = 0; r.lerY = 0;
        r.score = 0; r.concentration = 0.0; r.strandedCells = 0;
        r.groupingBonus = 0;
        r.allPlaced = false;
    ...
    {
        MaxRectsPack(ctx, gridW, gridH, order, target, abortFlag,
                     bestReserveX, bestReserveW, 1);  // BAF
        ++diagPackCalls;
        if (abortFlag && *abortFlag != 0)
        {
            Result r;
        ...
        if (ctx.placements.size() < bssfPl.size())
    ...
    ComputeLERCtx(ctx, &ctx.grid[0], gridW, gridH, seedLerA, seedLerW, seedLerH,
                  seedLerX, seedLerY);
    int seedStranded = 0;
    double seedConc = ComputeConcentrationAndStrandedCtx(
        ctx, gridW, gridH, seedLerX, seedLerY, seedLerW, seedLerH,
        seedStranded);
    int seedNumRot = CountRotated(seedPl);
    long long seedGrouping = ComputeGroupingBonus(seedPl, items, effGroupingPower);
    long long seedScore = ComputeScore(seedPl.size(), seedLerA, seedLerH,
                                       seedConc, target, seedNumRot,
                                       seedGrouping, seedStranded,
                                       effGroupingWeight, effFragWeight);
    ...
    unsigned int seed = (params && params->rngSeed != 0)
                      ? params->rngSeed
                      : ((unsigned int)(target * 2654435761u)
                         ^ (unsigned int)(items.size() * 40503u));
    LCG rng(seed);
    ...
    for (int restart = 0; restart < effectiveRestarts; ++restart)
    {
        ...
        if (restart == 0 && seedOrder != NULL)
            curOrder = *seedOrder;
        else
            curOrder = order;
        if (restart > 0)
            ShuffleGroups(curOrder, rng);

        // Pack the restart seed
        SkylinePack(ctx, gridW, gridH, curOrder, target, abortFlag,
                    bestReserveX, bestReserveW);
        ++diagPackCalls;
        if (abortFlag && *abortFlag != 0)
            break;
        BuildOccupancyGrid(ctx, gridW, gridH);
        ...
        ComputeLERCtx(ctx, &ctx.grid[0], gridW, gridH, curLerA, curLerW, curLerH,
                      curLerX, curLerY);
        int curStranded = 0;
        double curConc = ComputeConcentrationAndStrandedCtx(
            ctx, gridW, gridH, curLerX, curLerY, curLerW, curLerH,
            curStranded);
        ...
        long long curScore = ComputeScore(ctx.placements.size(), curLerA,
                                          curLerH, curConc, target,
        ...
        for (int iter = 0; iter < effIters; ++iter)
        {
            ...
            if (!enableRepairMove && moveRoll >= effRotateMax)
                moveRoll = rng.nextInt(effSwapMax > 0 ? effSwapMax : 1);
            ...
            else
            {
                ...
                if (bestStranded > 0 && !bestPl.empty())
                {
                    ...
                    if (repairGridDirty)
                    {
                        ...
                        FloodFillFromLer(ctx, gridW, gridH,
                                         bestLerX, bestLerY, bestLerW, bestLerH);
                    ...
                    if (strandedX >= 0)
                    {
                        ...
                        while (!ctx.floodStack.empty() && gapCells < 16)
                        {
                            ...
                            { ctx.visited[ci-1]=1; ctx.floodStack.push_back(ci-1); }
                            if (cx < gridW-1 && !ctx.visited[ci+1] && !ctx.grid[ci+1])
                            { ctx.visited[ci+1]=1; ctx.floodStack.push_back(ci+1); }
                            if (cy > 0 && !ctx.visited[ci-gridW] && !ctx.grid[ci-gridW])
                            { ctx.visited[ci-gridW]=1; ctx.floodStack.push_back(ci-gridW); }
                        ...
                        for (int ki = 0; ki < n; ++ki)
                        {
                            ...
                            bool fitsRotated = (curOrder[ki].canRotate
                                                && curOrder[ki].h <= gapW
                                                && curOrder[ki].w <= gapH);
            ...
            ComputeLERCtx(ctx, &ctx.grid[0], gridW, gridH, candLerA, candLerW, candLerH,
                          candLerX, candLerY);
            PROF_TICK(profLer);

            int candStranded = 0;
            double candConc = ComputeConcentrationAndStrandedCtx(
                ctx, gridW, gridH, candLerX, candLerY, candLerW, candLerH,
                candStranded);
            ...
            long long candScore = ComputeScore(ctx.placements.size(), candLerA,
                                               candLerH, candConc, target,
                                               candNumRot, candGrouping,
                                               candStranded,
                                               effGroupingWeight, effFragWeight);
            PROF_TICK(profScore);
            ...
            if (accept)
            {
                if (move.type == MOVE_REPAIR)
                    ++diagRepairMoveAccepts;
                curScore = candScore;
        ...
        {
            if (bestIterInRestart0 >= 0
                && bestIterInRestart0 < FAST_CONVERGE_ITER
                && bestConc > GOOD_CONC_THRESHOLD
    ...
    if (enableUnconstrainedFallback && bestReserveW > 0
        && !(abortFlag && *abortFlag != 0))
    {
    ...
    if (enableOptimizeGrouping)
        OptimizeGrouping(bestPl, items, effGroupingPower);
    ...
    result.score         = ComputeScore(bestPl.size(), bestLerA, bestLerH,
                                        bestConc, target, bestNumRot,

...
void SortWorker::ExecuteJob(Job* job, int workerIdx)
{
    ...
    if (!cs || cs->generation != ref.generation) goto done;
    ...
    // Nested scope keeps the vector out of goto done's bypass path.
    {
        ...
        Packer::TargetDim dimLocal = Packer::TARGET_H;
        {
            ...
            if (cs->generation != ref.generation) goto done;
        ...
        if (job->target == 0)
        {
                ...
                if (result.allPlaced && lerSide > t && lerSide <= maxSlots)
                {
                    ...
                    if (skipTo > t)
                    {
                        DebugLog("[StackSort] W" + IntToStr(workerIdx) + " [" + dimTag +
                                 "] skipping target=" + IntToStr(t + 1) + ".." + IntToStr(skipTo) +
            ...
            if (job->abortFlag == 0 && cs->generation == ref.generation)
            {
                ...
                if (numCandidates > 0)
                {
                    ...
                    InfoLog("[StackSort] W" + IntToStr(workerIdx) + " [" + dimTag + "] requeued " +
        ...
        else if (job->target > 0)
        {
            ...
            if (target < 1 || target > maxSlots) goto done;
            ...
            if (job->abortFlag != 0 || cs->generation != ref.generation) goto done;
    ...
    if (job->abortFlag != 0) summary << " (ABORTED)";
    InfoLog(summary.str());
}

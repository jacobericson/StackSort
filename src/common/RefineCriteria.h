...
static inline bool NeedsRefinement(const Packer::Result& r, Packer::TargetDim dim, int perpendicularGridExtent,
                                   int target, int totalFreeCells, int numItems = 0, int strandedMax = 0,
                                   int itemThreshold = REFINE_ITEM_THRESHOLD)
{
    if (!r.allPlaced) return true;

    int perpendicularLerSide = (dim == Packer::TARGET_H) ? r.lerWidth : r.lerHeight;
    ...
    if (strippable && perpendicularLerSide < perpendicularGridExtent) return true;

    if (r.concentration < 0.95) return true;

    if (r.strandedCells > strandedMax) return true;

    if (numItems > itemThreshold) return true;

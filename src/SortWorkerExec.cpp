...
static std::string BuildGridString(int gridW, int gridH, int reserveY,
...
{
    LARGE_INTEGER freq, totalStart, totalEnd;
    ...
    if (!cs || cs->generation != ref.generation)
        goto done;
    ...
    // Nested scope keeps the vector out of goto done's bypass path.
    {
    ...
    Packer::TargetDim dimLocal = Packer::TARGET_H;
    {
        ...
        if (cs->generation != ref.generation)
            goto done;
    ...
    if (job->target == 0)
    {
        ...
        for (int t = targetStart; t <= targetEnd; ++t)
        {
            ...
            int skipThreshold = (t <= maxLerSideCovered)
                              ? globalBestLERArea : 0;

            std::vector<Packer::Item> newBestOrder;
            Packer::Result result = Packer::PackAnnealed(
                gridW, gridH, packItemsLocal,
                dim, t, &job->abortFlag,
            ...
            if (!newBestOrder.empty())
                lastBestOrder.swap(newBestOrder);
            ...
            if (result.allPlaced)
            {
                if (result.lerArea > globalBestLERArea)
                    globalBestLERArea = result.lerArea;
                if (lerSide > maxLerSideCovered)
                    maxLerSideCovered = lerSide;
            }
            ...
            msg << "[StackSort] W" << workerIdx << " [" << dimTag
                << "] target=" << t
                << ": LER " << result.lerWidth << "x" << result.lerHeight
                << "=" << result.lerArea
                << " at (" << result.lerX << "," << result.lerY << ")";
            msg.precision(1);
            msg << std::fixed << ", " << ms << "ms"
                << ", conc ";
            msg.precision(2);
            msg << result.concentration
                << ", str " << result.strandedCells
                << ", grp " << result.groupingBonus
                << (result.allPlaced ? "" : " PARTIAL");
            DebugLog(msg.str());
            {
                int reserveLine = (dim == Packer::TARGET_H) ? (gridH - t) : gridH;
                DebugLog("\n" + BuildGridString(gridW, gridH, reserveLine,
                                                 result, packItemsLocal));
            }

            {
                ...
                {
                    if (!newBestOrder.empty())
                        cs->bestOrders[t - 1] = newBestOrder;
                    else
                        cs->bestOrders[t - 1] = lastBestOrder;
                }
            }
                ...
                {
                    {
                        ...
                        if (skip - 1 < (int)cs->bestOrders.size())
                            cs->bestOrders[skip - 1] = cs->bestOrders[t - 1];
                    }
                    AdvanceComputedUpTo(&cs->computedUpTo, (LONG)skip);

                    if (t == 1 && skip - 1 < 64)
                        skipFromLoosest[skip - 1] = true;
                }
                if (skipTo > t)
                {
                    DebugLog("[StackSort] W" + IntToStr(workerIdx) + " [" + dimTag
                             + "] skipping target=" + IntToStr(t + 1)
                             + ".." + IntToStr(skipTo)
                             + " (covered by target=" + IntToStr(t) + ")");
                    t = skipTo;
                }
            }
        }
        ...
        {
            struct RefineCandidate { int t; long long score; };
            ...
            // safe for any plausible Kenshi grid (max ~20 per side).
            {
                ...
                {
                    if (rt - 1 < 64 && skipFromLoosest[rt - 1])
                        continue;
                    if (NeedsRefinement(cs->results[rt - 1], dim, perpendicular,
                                        rt, totalFreeCells,
                                        (int)packItemsLocal.size()))
                    {
                        if (numCandidates >= 64)
                        {
                            ErrorLog("[StackSort] refinement candidate overflow"
            ...
            if (numCandidates > 0)
            {
                ...
                InfoLog("[StackSort] W" + IntToStr(workerIdx) + " [" + dimTag
                        + "] requeued " + IntToStr(numCandidates)
    ...
    else if (job->target > 0)
    {
        ...
        bool hasSeed = false;
        {
            ...
            if (target - 1 < (int)cs->bestOrders.size()
                && !cs->bestOrders[target - 1].empty())
            {
        ...
        if (refined.allPlaced && refined.score > oldScore)
        {
            ...
            rmsg << "[StackSort] W" << workerIdx
                 << " [" << dimTag << "] REFINED target=" << target
                 << ": score " << oldScore << " -> " << refined.score;
            rmsg.precision(1);
            rmsg << std::fixed << " (" << ms << "ms)";
            DebugLog(rmsg.str());
            {
                int reserveLine = (dim == Packer::TARGET_H)
                                ? (gridH - target) : gridH;
                DebugLog("\n" + BuildGridString(gridW, gridH, reserveLine,
                                                 refined, packItemsLocal));
            }
        }
        else
        {
    ...
    double totalMs = (double)(totalEnd.QuadPart - totalStart.QuadPart)
                   / (double)freq.QuadPart * 1000.0;

...
int SortWorker::AllocTracking(InventoryGUI* gui)
{
    ...
    ErrorLog("[StackSort] All GUI tracking slots full (max "
             + IntToStr(MAX_GUIS) + ")");
    return -1;
}
...
void SortWorker::ScanInventory(int trackIdx, Inventory* inv,
                               bool* outNeedsCompute)
{
         ...
         it != sections.end(); ++it)
    {
        ...
        {
            DebugLog("[StackSort] Cache hit: section=" + IntToStr((int)(intptr_t)section)
                     + " '" + section->name + "'"
        ...
        DebugLog("[StackSort] Cache miss: section=" + IntToStr((int)(intptr_t)section)
                 + " '" + section->name + "'"
...
void SortWorker::SnapshotSections(int trackIdx, InventoryGUI* gui,
                                  bool* outNeedsCompute)
{
    GUITracking& t = s_guiTracking[trackIdx];
    t.numSections = 0;
    ...
    if (backpack && backpack->inventory)
    {
        ...
        if (t.numTrackedInv < 2)
            t.trackedInv[t.numTrackedInv++] = backpack->inventory;
    }
}
...
void SortWorker::EnqueueJob(int trackIdx)
{
...

    {
        ...
        for (int i = 0; i < t.numSections; ++i)
        {
            ...
            int maxSlots = (dim == Packer::TARGET_H) ? (cs->gridH - 1)
                                                     : (cs->gridW - 1);
            if (maxSlots < 1)
                continue;
    ...
    if (numEnqueued > 0)
        ReleaseSemaphore(s_workSemaphore, numEnqueued, NULL);
    ...
    for (int i = 0; i < t.numSections; ++i)
    {
        ...
        if (cs)
        {
            msg += IntToStr(cs->gridW) + "x" + IntToStr(cs->gridH)
                 + "/" + IntToStr(cs->itemCount) + " items";
        }
...
void SortWorker::AbortJobsForGUI(InventoryGUI* gui)
{
    ...
    for (int w = 0; w < NUM_WORKERS; ++w)
    {
        ...
        if (active && active->sourceGUI == gui)
            InterlockedExchange(&active->abortFlag, 1);
    }
}
    ...
    while (s_shutdown == 0)
    {
        ...
        Job* job = NULL;
        {
            ...
            if (!s_jobQueue.empty())
            {
                ...
                {
                    if (s_jobQueue[i]->priority < s_jobQueue[bestIdx]->priority)
                        bestIdx = (int)i;
                }
...
void SortWorker::Init()
{
    ...
    {
        s_activeJobs[i] = NULL;
        s_threads[i] = CreateThread(NULL, 0, WorkerProc,
                                    (LPVOID)(intptr_t)i, 0, NULL);
        if (!s_threads[i])
            ErrorLog("[StackSort] Failed to create worker thread "
                     + IntToStr(i));
    }
    InfoLog("[StackSort] " + IntToStr(NUM_WORKERS)
...
void SortWorker::Shutdown()
{
    ...
    if (s_workSemaphore)
        ReleaseSemaphore(s_workSemaphore, NUM_WORKERS, NULL);
...
void SortWorker::OnInventoryOpened(InventoryGUI* gui)
{
    ...
    if (s_guiTracking[idx].numSections == 0)
        return;  // no eligible sections (keep tracking to prevent per-frame retry)

    InfoLog("[StackSort] Inventory opened (cache: "
            + IntToStr(ResultCache::Size()) + " entries)");
    ...
    for (int i = 0; i < MAX_GUIS; ++i)
    {
        if (!s_guiTracking[i].gui)
            continue;
        for (int j = 0; j < s_guiTracking[i].numSections; ++j)
        {
            if (numRefs < MAX_GUIS * MAX_SECTIONS)
                allRefs[numRefs++] = s_guiTracking[i].sectionKeys[j];
        }
    }
...
void SortWorker::OnInventoryClosed(InventoryGUI* gui)
{
    ...
    InfoLog("[StackSort] Inventory closed (cache: "
            + IntToStr(ResultCache::Size()) + " entries)");
}
...
void SortWorker::PollWorkerStart()
{
    for (int i = 0; i < MAX_GUIS; ++i)
    {
        ...
        if (bodyInv)
            ScanInventory(i, bodyInv, &needsCompute);

        ContainerItem* backpack = t.gui->getBackpack();
        if (backpack && backpack->inventory)
            ScanInventory(i, backpack->inventory, &needsCompute);
...
bool SortWorker::GetResult(InventorySection* section, int target,
...
bool SortWorker::GetBestResult(InventorySection* section, int target,
                               Packer::Result& outResult, int* outSourceTarget)
{
    CachedSection* cs = ResultCache::Find(section);
    if (!cs || cs->valid == 0)
        return false;
...
bool SortWorker::IsSectionActive(InventorySection* section)
{
    ...
    for (size_t i = 0; i < s_jobQueue.size(); ++i)
    {
        if (s_jobQueue[i]->section.key == section)
            return true;
    }
    for (int w = 0; w < NUM_WORKERS; ++w)
    {
        Job* j = (Job*)s_activeJobs[w];
        if (j && j->section.key == section)
            return true;
    }
    return false;
}
...
void SortWorker::ToggleDim()
{
    Packer::TargetDim newDim;
    {
        ...
        newDim = (GetDim() == Packer::TARGET_H)
                                 ? Packer::TARGET_W : Packer::TARGET_H;
    ...
    InfoLog(std::string("[StackSort] Target dim toggled to ")
            + (newDim == Packer::TARGET_H ? "H" : "W"));
}

...
struct CallbackGuard
{
    ...
    CallbackGuard(InventorySection* s)
        : section(s), saved(s->callbackObject)
    {
...
struct ApplyGuard
{
    ApplyGuard()  { SortWorker::SetApplying(true); }
    ~ApplyGuard() { SortWorker::SetApplying(false); }
};
...
static void applyLayout(InventorySection* section,
                        const Packer::Result& result,
                        const std::vector<Item*>& itemPtrs)
{
    ...
    if (StackSort_RotateAvailable())
    {
        for (size_t i = 0; i < result.placements.size(); ++i)
        {
            ...
            if (item->itemWidth == p.w && item->itemHeight == p.h)
                continue; // dims already match — no rotation needed

            bool currentlyRotated = StackSort_IsRotated(item);
            if (!StackSort_SetRotated(item, !currentlyRotated))
            {
                InfoLog("[StackSort] SetRotated failed for item "
                         + IntToStr(p.id) + ", will auto-place");
    ...
    for (size_t i = 0; i < result.placements.size(); ++i)
    {
        const Packer::Placement& p = result.placements[i];
        if (rotationFailed[i])
            section->addItem(itemPtrs[p.id], 1);
        else
            section->_NV__addItem(itemPtrs[p.id], p.x, p.y);
    }

    section->recalculateTotalWeight();
}

void StackSortUI::ApplySort(InventoryGUI* gui, int target,
                            int& outAppliedLerSide, int& outSourceTarget)
{
    ...
    for (lektor<InventorySection*>::iterator it = sections.begin();
         it != sections.end(); ++it)
    {
        ...
        const Ogre::vector<InventorySection::SectionItem>::type& sectionItems =
            section->getItems();
        ...
        Packer::ComputeLER(beforeGrid, gridW, gridH,
                           beforeLerArea, beforeLerW, beforeLerH,
                           beforeLerX, beforeLerY);

        // Try best precomputed result across all target values
        int sourceTarget = 0;
        Packer::Result precomputed;
        bool hasPrecomputed =
            SortWorker::GetBestResult(section, clamped, precomputed, &sourceTarget);

        if (hasPrecomputed && precomputed.allPlaced)
        {
            std::vector<Item*> ptrs;
            if (SortWorker::GetItemPtrs(section, ptrs)
                && ptrs.size() == precomputed.placements.size()
                && Packer::ValidatePlacements(gridW, gridH,
                                              precomputed.placements))
            {
                std::string tStr = std::string("[") + dimTag + "] target="
                                 + IntToStr(clamped);
                if (sourceTarget != clamped)
                    tStr += " (via target=" + IntToStr(sourceTarget) + ")";

                InfoLog("[StackSort] Applying " + tStr
                         + " for '" + section->name + "'"
                         + " (" + IntToStr((int)ptrs.size()) + " items)"
                         + ", LER " + IntToStr(precomputed.lerWidth)
                         + "x" + IntToStr(precomputed.lerHeight)
                         + "=" + IntToStr(precomputed.lerArea)
                         + " at (" + IntToStr(precomputed.lerX)
                         + "," + IntToStr(precomputed.lerY) + ")"
                         + " (was " + IntToStr(beforeLerW)
                         + "x" + IntToStr(beforeLerH)
                         + "=" + IntToStr(beforeLerArea) + ")");

                applyLayout(section, precomputed, ptrs);
                int lerSide = (dim == Packer::TARGET_H) ? precomputed.lerHeight
                                                         : precomputed.lerWidth;
                if (lerSide > outAppliedLerSide)
                    outAppliedLerSide = lerSide;
                if (sourceTarget > maxSourceTargetThisCall)
                    maxSourceTargetThisCall = sourceTarget;
                anySorted = true;
                continue;
            }
        }
        ...
        InfoLog(std::string("[StackSort] Sync fallback [") + dimTag
                 + "] target=" + IntToStr(clamped)
                 + " for '" + section->name + "'"
                 + " (" + IntToStr(gridW) + "x" + IntToStr(gridH)
                 + ", " + IntToStr((int)packItems.size()) + " items)");

        QueryPerformanceCounter(&t0);
        Packer::Result result = Packer::Pack(gridW, gridH, packItems,
                                             dim, clamped);
        ...
        if (!result.allPlaced)
        {
            InfoLog("[StackSort] Not all items placed ("
                     + IntToStr((int)result.placements.size()) + "/"
                     + IntToStr((int)packItems.size())
        ...
        logMsg << "[StackSort] Packed " << packItems.size() << " items in "
               << std::fixed;
        logMsg.precision(2);
        logMsg << ms << "ms"
               << ", LER " << result.lerWidth << "x" << result.lerHeight
               << "=" << result.lerArea
               << " (was " << beforeLerW << "x" << beforeLerH
               << "=" << beforeLerArea << ")"
               << ", score " << result.score;
        DebugLog(logMsg.str());

        applyLayout(section, result, itemPtrs);
        int lerSide = (dim == Packer::TARGET_H) ? result.lerHeight
                                                 : result.lerWidth;
        if (lerSide > outAppliedLerSide)
            outAppliedLerSide = lerSide;
        // Sync fallback has no source slot — leave outSourceTarget at the
        // current target so the W-mode skip is a no-op.
        if (clamped > maxSourceTargetThisCall)
            maxSourceTargetThisCall = clamped;
        anySorted = true;
    }
...
bool StackSortUI::ConsolidateStacks(InventorySection* section)
{
    ...
    const Ogre::vector<InventorySection::SectionItem>::type& liveItems =
        section->getItems();
    ...
    {
        if (liveItems[i].item)
            snapshot.push_back(liveItems[i].item);
    }
...
void StackSortUI::CaptureOriginalPositions(InventoryGUI* gui)
{
         ...
         it != sections.end(); ++it)
    {
        ...
        if (!section->enabled)
            continue;
        if (section->getItems().empty())
            continue;

        CachedSection* cs = ResultCache::Find(section);
        if (!cs || cs->hasOriginalPositions)
            continue;
...
bool StackSortUI::RevertInventory(InventoryGUI* gui)
{
         ...
         it != sections.end(); ++it)
    {
        ...
        const Ogre::vector<InventorySection::SectionItem>::type& currentItems =
            section->getItems();
        ...
        for (size_t i = 0; i < currentItems.size(); ++i)
        {
            if (currentItems[i].item)
                currentPtrs.push_back(currentItems[i].item);
        }
        ...
        {
            if (!matched[j])
                stragglers.push_back(currentPtrs[j]);
        }

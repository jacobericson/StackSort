...
{
    fprintf(stderr,
        "Usage: stacksort_bench --corpus DIR --config FILE --out FILE [options]\n"
...
int main(int argc, char** argv)
{
    ...
    std::string configDir = ".";
    {
        ...
        if (slash != std::string::npos)
            configDir = args.configFile.substr(0, slash);
    }
    ...
    for (size_t ii = 0; ii < instancePaths.size(); ++ii)
    {
        ...
        for (int seedIdx = 0; seedIdx < args.numSeeds; ++seedIdx)
        {
            for (int target = startT; target <= endT; ++target)
            {
                ...
                Packer::Result firstPass = Packer::PackAnnealed(
                    inst.gridW, inst.gridH, inst.items,
                    Packer::TARGET_H, target,
                    NULL, NULL, &firstPassBest, 0,
                    &runParams, &firstDiag);
                QueryPerformanceCounter(&tFP1);
                double firstPassMs =
                    (double)(tFP1.QuadPart - tFP0.QuadPart) * 1000.0
                ...
                if (args.refineMode)
                {
                    ...
                    int strandedMax    = (targetCfg.refineStrandedMax >= 0)
                                         ? targetCfg.refineStrandedMax : 0;
                    int itemThreshold  = (targetCfg.refineItemThreshold >= 0)
                                         ? targetCfg.refineItemThreshold : REFINE_ITEM_THRESHOLD;

                    if (targetCfg.refineAlways == 1
                        || NeedsRefinement(firstPass, Packer::TARGET_H,
                                           perpendicular, target, totalFreeCells,
                                           (int)inst.items.size(),
                                           strandedMax, itemThreshold))
                    {
                        refineTriggered = 1;

                        Packer::SearchParams refineParams = runParams;
                        refineParams.numRestarts      = (targetCfg.refineRestarts > 0) ? targetCfg.refineRestarts : REFINE_RESTARTS;
                        refineParams.itersPerRestart  = (targetCfg.refineIters    > 0) ? targetCfg.refineIters    : REFINE_ITERS;
                        refineParams.lahcHistoryLen   = (targetCfg.refineLahcHist > 0) ? targetCfg.refineLahcHist : REFINE_LAHC_HIST;
                        refineParams.plateauThreshold = (targetCfg.refinePlateau  > 0) ? targetCfg.refinePlateau  : REFINE_PLATEAU;
                        ...
                        Packer::Result refined = Packer::PackAnnealed(
                            ...
                            Packer::TARGET_H, target,
                            NULL,
                            firstPassBest.empty() ? NULL : &firstPassBest,
                            &refineBest,
                            0,
                            &refineParams,
                            &refineDiag);
                        ...
                        refineMs =
                            (double)(tR1.QuadPart - tR0.QuadPart) * 1000.0
                            / (double)freq.QuadPart;

                        if (refined.allPlaced
                            && refined.score > firstPass.score)
                        {
                            finalResult    = refined;
                ...
                int resolvedGW = (runParams.scoringGroupingWeight > 0)
                                 ? runParams.scoringGroupingWeight
                                 : Packer::DEFAULT_GROUPING_WEIGHT;
                int resolvedFW = (runParams.scoringFragWeight > 0)
                                 ? runParams.scoringFragWeight
                                 : Packer::DEFAULT_FRAG_WEIGHT;
                ...
                int resolvedGPQ = (runParams.groupingPowerQuarters >= 1
                                   && runParams.groupingPowerQuarters <= 8)
                                  ? runParams.groupingPowerQuarters
                                  : Packer::DEFAULT_GROUPING_POWER_QUARTERS;
                int resolvedSWC = (runParams.skylineWasteCoef >= 1)
                                  ? runParams.skylineWasteCoef
        ...
        fprintf(stdout, "[%s] %dx%d, %d items, density %.2f, targets %d..%d\n",
                inst.name.c_str(), inst.gridW, inst.gridH,
                (int)inst.items.size(), density, startT, endT);
    }

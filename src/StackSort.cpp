...
typedef unsigned long (*AK_PostEvent_t)(
...
void StackSort_InitRotateAPI()
{
    ...
    if (ver < 1)
    {
        ErrorLog("[StackSort] KenshiRotate API version " + IntToStr(ver)
                 + " < 1 (rotation disabled)");
        return;
    }

    s_krAvailable = true;
    InfoLog("[StackSort] KenshiRotate detected (API v" + IntToStr(ver)
             + ", rotation enabled)");
}
...
{
    if (s_krAvailable && s_krRefreshVisuals)
        s_krRefreshVisuals((void*)gui);
}
...
{
    if (s_akPostEvent)
        s_akPostEvent("General_Click", 0x6E, 0, NULL, NULL, 0, NULL, 0);
}
...
static void hook_autoArrangeButton(InventoryGUI* thisptr, MyGUI::Widget* sender)
{
    ...
    {
        if (orig_autoArrangeButton)
            orig_autoArrangeButton(thisptr, sender);
        return;
    }
...
static void hook_show(InventoryGUI* thisptr, bool on)
{
    if (!on)
    {
        ...
        if (thisptr->childInventory)
            StackSortUI::OnClose(thisptr->childInventory);
    }
...
static void hook_update(InventoryGUI* thisptr)
{
    ...
    if (!s_baseVtable && thisptr->childInventory != NULL)
        s_baseVtable = *(void***)thisptr;

    // Skip trader GUIs (different vtable than base InventoryGUI)
    if (isTraderGUI(thisptr))
        return;
    ...
    if (SortWorker::HasContext(thisptr)
        && !SortWorker::ContextMatchesInventory(thisptr))
    {
    ...
    if (!SortWorker::HasContext(thisptr))
        SortWorker::OnInventoryOpened(thisptr);
...
__declspec(dllexport) void startPlugin()
{
    ...
    if (KenshiLib::SUCCESS != KenshiLib::AddHook(
        (void*)KenshiLib::GetRealAddress(&InventoryGUI::autoArrangeButton),
        (void*)hook_autoArrangeButton,
        (void**)&orig_autoArrangeButton))
    {
    ...
    if (KenshiLib::SUCCESS != KenshiLib::AddHook(
        (void*)KenshiLib::GetRealAddress(&InventoryGUI::_NV_show),
        (void*)hook_show,
        (void**)&orig_show))
    {
    ...
    if (KenshiLib::SUCCESS != KenshiLib::AddHook(
        (void*)KenshiLib::GetRealAddress(&Inventory::_NV__sectionAddItemCallback),
        (void*)hook_sectionAddItemCallback,
        (void**)&orig_sectionAddCb))
    {
    ...
    if (KenshiLib::SUCCESS != KenshiLib::AddHook(
        (void*)KenshiLib::GetRealAddress(&Inventory::_NV__sectionRemoveItemCallback),
        (void*)hook_sectionRemoveItemCallback,
    ...
    // Resolve AK::SoundEngine::PostEvent from kenshi_x64.exe
    {
        ...
        if (hGame)
        {
            s_akPostEvent = (AK_PostEvent_t)GetProcAddress(hGame,
                "?PostEvent@SoundEngine@AK@@YAKPEBD_KKP6AXW4AkCallbackType@@"

...
void Packer::CollectAdjacentPids(
    ...
    int* adjPids, int& numAdj, int maxAdj)
{
    for (int i = 0; i < count; ++i)
    {
        ...
        if (pidx < 0 || items[ctx.placements[pidx].id].itemTypeId != curType)
            continue;
        bool dup = false;
        for (int k = 0; k < numAdj; ++k)
            if (adjPids[k] == pidx) { dup = true; break; }
        if (!dup && numAdj < maxAdj)
            adjPids[numAdj++] = pidx;
    }
}
    ...
    int reserveX, int reserveW)
{
    ...
    for (size_t idx = 0; idx < items.size(); ++idx)
    {
            ...
            for (int ori = 0; ori < numOri; ++ori)
            {
                ...
                for (size_t si = 0; si < ctx.skyline.size(); ++si)
                {
                    ...
                    if (reserveW > 0)
                    {
                        ...
                        int ceiling = gridH;
                        if (x < reserveX + reserveW && x + iw > reserveX)
                            ceiling = reserveY;
                        if (maxY + ih > ceiling)
                            continue;
                    }
                    ...
                    int numAdj = 0;
                    if (x > 0)
                        CollectAdjacentPids(ctx, items, curType,
                            maxY * gridW + (x - 1), gridW, ih,
                            adjPids, numAdj, MAX_ADJ);
                    if (x + iw < gridW)
                        CollectAdjacentPids(ctx, items, curType,
                            maxY * gridW + (x + iw), gridW, ih,
                            adjPids, numAdj, MAX_ADJ);
                    if (maxY > 0)
                        CollectAdjacentPids(ctx, items, curType,
                            (maxY - 1) * gridW + x, 1, iw,
                    ...
                    bool isBetter = (maxY < bestY)
                        || (maxY == bestY && combined < bestCombined)
                        || (maxY == bestY && combined == bestCombined
                            && iw >= ih && !(bestW >= bestH));
        ...
        size_t insertPos = 0;
        while (insertPos < ctx.skylineTmp.size()
               && ctx.skylineTmp[insertPos].x < placeLeft)
            ++insertPos;
        ...
        {
            if (!ctx.skyline.empty()
                && ctx.skyline.back().y == ctx.skylineTmp[si].y

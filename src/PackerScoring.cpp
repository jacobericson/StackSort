...
static long long applyGroupingPower(int b, int quarters)
{
    ...
    {
        case 4: return b;                              // b^1
        case 6: return (long long)b * isqrt(b);        // b^1.5 — DEFAULT, must match legacy bit-for-bit
        case 8: return (long long)b * b;               // b^2
    }

    long long bq = 1;
    for (int i = 0; i < quarters; ++i) bq *= b;
...
void Packer::ComputeLERCtx(PackContext& ctx,
...
double Packer::ComputeConcentrationAndStrandedCtx(
    ...
    int& outStrandedCells)
{
    ...
    for (int i = 0; i < totalCells; ++i)
    {
        ...
        while (!ctx.floodStack.empty())
        {
            ...
            if (cx >= lerX && cx < lerXEnd && cy >= lerY && cy < lerYEnd)
                hasLer = 1;
    ...
    {
        if (!ctx.regionHasLer[r])
            stranded += ctx.regionInterior[r];
    }
...
void Packer::FloodFillFromLer(PackContext& ctx, int gridW, int gridH,
                              int lerX, int lerY, int lerW, int lerH)
{
    int totalCells = gridW * gridH;
    if (totalCells == 0)
...
void Packer::ComputeLER(const std::vector<unsigned char>& grid,
                        int gridW, int gridH,
                        int& outArea, int& outWidth, int& outHeight,
                        int& outX, int& outY)
{
    ...
    ComputeLERCtx(ctx, &grid[0], gridW, gridH,
                  outArea, outWidth, outHeight, outX, outY);
}

long long Packer::ComputeScore(size_t numPlaced, int lerArea, int lerHeight,
...
void Packer::BuildAdjGraph(AdjGraph& g,
...
long long Packer::ComputeGroupingBonusAdj(
    const std::vector<Placement>& placements,
    ...
    int groupingPowerQuarters)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        for (int k = 0; k < g.count[i]; ++k)
        {
            ...
            if (items[placements[j].id].itemTypeId == typeA)
                uf_unite(parent, i, j);
        }
    }
...
long long Packer::ComputeGroupingBonus(const std::vector<Placement>& placements,
                                       const std::vector<Item>& items,
                                       int groupingPowerQuarters)
{
    int n = (int)placements.size();
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        for (int j = i + 1; j < n; ++j)
        {
            if (items[placements[j].id].itemTypeId != typeA)
                continue;
            if (SharedBorder(placements[i], placements[j]) > 0)
                uf_unite(parent, i, j);
        }
    }
...
long long Packer::ComputeGroupingBordersRaw(const std::vector<Placement>& placements,
                                            const std::vector<Item>& items)
{
    int n = (int)placements.size();
    if (n <= 1 || n > 256)
...
void Packer::OptimizeGrouping(std::vector<Placement>& placements,
                               const std::vector<Item>& items,
                               int groupingPowerQuarters)
{
    int n = (int)placements.size();
    ...
    while (runStart < n)
    {
        ...
        while (runEnd < n
               && footprintKey[sortedIdx[runEnd]] == footprintKey[sortedIdx[runStart]])
            ++runEnd;
    ...
    while (improved)
    {
        ...
        for (int ci = 0; ci < numCandidates; ++ci)
        {
            ...
            if (items[placements[pi].id].itemTypeId ==
                items[placements[pj].id].itemTypeId)
                continue;
...
bool Packer::ValidatePlacements(int gridW, int gridH,
                                const std::vector<Placement>& placements)
{

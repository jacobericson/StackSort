...
public:
    ...
    enum TargetDim { TARGET_H = 0, TARGET_W = 1 };
    ...
    static Result Pack(int gridW, int gridH,
    ...
    static void ComputeLER(const std::vector<unsigned char>& grid,
                           int gridW, int gridH,
                           int& outArea, int& outWidth, int& outHeight,
                           int& outX, int& outY);

    // Check that no cell is doubly-occupied and all coords are in bounds.
    static bool ValidatePlacements(int gridW, int gridH,
    ...
    struct SearchParams
    {
        ...
        SearchParams()
            : numRestarts(-1), itersPerRestart(-1)
            , lahcHistoryLen(-1), plateauThreshold(-1)
            , rngSeed(0)
            , enableBafSeed(-1), enableUnconstrainedFallback(-1)
            , enableOptimizeGrouping(-1), enableFastConverge(-1)
            , enableRepairMove(-1), enablePreReservation(-1)
            , moveSwapMax(-1), moveInsertMax(-1), moveRotateMax(-1)
            , scoringGroupingWeight(-1), scoringFragWeight(-1)
            , groupingPowerQuarters(-1)
            , skylineWasteCoef(-1)
        {}

        // Equivalent to default construction; explicit form for readability
        // at caller sites that override a subset of fields.
        static SearchParams defaults() { return SearchParams(); }
    };
    ...
    struct PackDiagnostics
    {
        ...
        PackDiagnostics()
            : bestFoundIter(0), bestFoundRestart(0), plateauBreaks(0)
            , lahcItersExecuted(0), packCalls(0)
            , unconstrainedFallbackWon(false)
            , greedySeedScore(0), greedySeedLerArea(0)
            , repairMoveAttempts(0), repairMoveHits(0), repairMoveAccepts(0)
            , groupingBordersRaw(0)
#ifdef STACKSORT_PROFILE
            , profCyclesMoveGen(0), profCyclesSkylinePack(0), profCyclesLer(0)
            , profCyclesConcentration(0), profCyclesGrouping(0)
            , profCyclesStranded(0), profCyclesScore(0), profCyclesAccept(0)
#endif
        {}
    };
    ...
    static Result PackAnnealed(int gridW, int gridH,
                               const std::vector<Item>& items,
                               TargetDim dim, int target,
                               volatile long* abortFlag = NULL,
                               const std::vector<Item>* seedOrder = NULL,
                               std::vector<Item>* outBestOrder = NULL,
                               int skipLAHCIfAreaBelow = 0,
                               const SearchParams* params = NULL,
                               PackDiagnostics* outDiag = NULL);
...
private:
    ...
    static Result PackH(int gridW, int gridH,
                        const std::vector<Item>& items, int target,
                        volatile long* abortFlag = NULL);

    static Result PackAnnealedH(int gridW, int gridH,
                                const std::vector<Item>& items, int target,
                                volatile long* abortFlag = NULL,
                                const std::vector<Item>* seedOrder = NULL,
                                std::vector<Item>* outBestOrder = NULL,
    ...
    static void InitPackContext(PackContext& ctx, int gridW, int gridH,
                                int numItems);
    ...
    static void MaxRectsPack(PackContext& ctx,
                             int gridW, int gridH,
                             const std::vector<Item>& items,
                             int target,
                             volatile long* abortFlag = NULL,
                             int reserveX = 0, int reserveW = 0,
                             int heuristic = 0);
    ...
    // Writes results into ctx.placements.
    static void SkylinePack(PackContext& ctx,
                            int gridW, int gridH,
                            const std::vector<Item>& items,
                            int target,
                            volatile long* abortFlag = NULL,
    ...
    static void ComputeLERCtx(PackContext& ctx,
    ...
    static double ComputeConcentrationAndStrandedCtx(
        PackContext& ctx, int gridW, int gridH,
        int lerX, int lerY, int lerW, int lerH,
    ...
    static void FloodFillFromLer(PackContext& ctx, int gridW, int gridH,
                                 int lerX, int lerY, int lerW, int lerH);
    ...
    static long long ComputeScore(size_t numPlaced, int lerArea, int lerHeight,
                                  double concentration, int target,
                                  int numRotated, long long groupingBonus,
                                  int strandedCells,
                                  int groupingWeight, int fragWeight);
    ...
    static long long ComputeGroupingBonus(const std::vector<Placement>& placements,
                                          const std::vector<Item>& items,
                                          int groupingPowerQuarters);
    ...
    static void OptimizeGrouping(std::vector<Placement>& placements,
                                 const std::vector<Item>& items,
                                 int groupingPowerQuarters);
    ...
    static void CollectAdjacentPids(
        const PackContext& ctx, const std::vector<Item>& items,
        int curType, int start, int step, int count,
        int* adjPids, int& numAdj, int maxAdj);

    // MAXRECTS placement heuristic helpers (used by MaxRectsPack).
    // aboveReserveY >= 0: only rects where item fits above that Y.
    static void FindBestBSSF(const std::vector<Rect>& freeRects,
                             const Item& item, int numOri, int aboveReserveY,
                             int& bestShortSide, int& bestLongSide,
                             int& bestIndex, int& bestW, int& bestH,
                             bool& bestRotated);
    static void FindBestBAF(const std::vector<Rect>& freeRects,
                            const Item& item, int numOri, int aboveReserveY,
                            long long& bestArea, int& bestShortSide,
                            int& bestIndex, int& bestW, int& bestH,
                            bool& bestRotated);
    ...
    static void BuildAdjGraph(AdjGraph& g,
                              const std::vector<Placement>& placements);

    static long long ComputeGroupingBonusAdj(
        const std::vector<Placement>& placements,
        const std::vector<Item>& items,
        const AdjGraph& g, int n,
        int groupingPowerQuarters);

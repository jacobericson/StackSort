...
void Packer::PruneFreeRects(PackContext& ctx)
{
    ...
    {
        if (!ctx.dead[read])
            ctx.freeRects[write++] = ctx.freeRects[read];
    }
...
void Packer::FindBestBSSF(
    const std::vector<Rect>& freeRects,
    const Item& item, int numOri, int aboveReserveY,
    int& bestShortSide, int& bestLongSide,
    int& bestIndex, int& bestW, int& bestH, bool& bestRotated)
{
    for (size_t fi = 0; fi < freeRects.size(); ++fi)
    {
        ...
        for (int ori = 0; ori < numOri; ++ori)
        {
            ...
            if (fr.w < tryW || fr.h < tryH)
                continue;
            if (aboveReserveY >= 0 && fr.y + tryH > aboveReserveY)
                continue;
            ...
            bool isBetter = (shortSide < bestShortSide)
                || (shortSide == bestShortSide && longSide < bestLongSide)
                || (shortSide == bestShortSide && longSide == bestLongSide
                    && tryW >= tryH && !(bestW >= bestH));
    ...
    int& bestIndex, int& bestW, int& bestH, bool& bestRotated)
{
    for (size_t fi = 0; fi < freeRects.size(); ++fi)
    {
        ...
        for (int ori = 0; ori < numOri; ++ori)
        {
            ...
            bool isBetter = (area < bestArea)
                || (area == bestArea && shortSide < bestShortSide)
                || (area == bestArea && shortSide == bestShortSide
                    && tryW >= tryH && !(bestW >= bestH));
...
void Packer::MaxRectsPack(PackContext& ctx,
    int gridW, int gridH, const std::vector<Item>& items,
    int target, volatile long* abortFlag,
    int reserveX, int reserveW, int heuristic)
{
    ...
    for (size_t idx = 0; idx < items.size(); ++idx)
    {
        ...
        if (heuristic == 1)
        {
            ...
            if (reserveW <= 0)
                FindBestBAF(ctx.freeRects, item, numOri, reserveY,
                            bestArea, bestShortSide,
                            bestIndex, bestW, bestH, bestRotated);
            ...
            if (bestIndex < 0)
            {
                ...
                FindBestBAF(ctx.freeRects, item, numOri, -1,
                            bestArea, bestShortSide,
                            bestIndex, bestW, bestH, bestRotated);
            }
        }
        else
        {
            ...
            if (reserveW <= 0)
                FindBestBSSF(ctx.freeRects, item, numOri, reserveY,
                             bestShortSide, bestLongSide,
                             bestIndex, bestW, bestH, bestRotated);
            ...
            if (bestIndex < 0)
            {
                ...
                FindBestBSSF(ctx.freeRects, item, numOri, -1,
                             bestShortSide, bestLongSide,
                             bestIndex, bestW, bestH, bestRotated);
            }
        }
        ...
        if (bestIndex < 0)
            continue; // Item doesn't fit — skip

...
Packer::Result Packer::Pack(int gridW, int gridH,
                            const std::vector<Item>& items,
                            TargetDim dim, int target,
                            volatile long* abortFlag)
{
    if (dim == TARGET_H)
        return PackH(gridW, gridH, items, target, abortFlag);
...
Packer::Result Packer::PackH(int gridW, int gridH,
                             ...
                             volatile long* abortFlag)
{
    ...
    ComputeLERCtx(ctx, &ctx.grid[0], gridW, gridH,
                  result.lerArea, result.lerWidth, result.lerHeight,
                  result.lerX, result.lerY);
    result.concentration = ComputeConcentrationAndStrandedCtx(
        ctx, gridW, gridH,
        result.lerX, result.lerY, result.lerWidth, result.lerHeight,
        result.strandedCells);

    // Score — PackH has no SearchParams plumbing, so use compile-time defaults.
    int numRot = CountRotated(result.placements);
    long long grouping = ComputeGroupingBonus(result.placements, items,
                                              Packer::DEFAULT_GROUPING_POWER_QUARTERS);
    result.groupingBonus = grouping;
    result.score = ComputeScore(result.placements.size(), result.lerArea,
                                result.lerHeight, result.concentration, target,
                                numRot, grouping, result.strandedCells,

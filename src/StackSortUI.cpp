...
int StackSortUI::FindUIState(InventoryGUI* gui)
{
    ...
    {
        if (s_uiStates[i].initialized && s_uiStates[i].gui == gui)
            return i;
    }
    return -1;
}
...
void StackSortUI::UpdateCaption(int uiIdx)
{
    ...
    if (ui.sortButton)
        ui.sortButton->setCaption(MakeCaption(ui.currentTarget));
    if (ui.dimButton)
        ui.dimButton->setCaption(
            (SortWorker::GetDim() == Packer::TARGET_H) ? "H" : "W");
}
...
int StackSortUI::FindMaxTarget(InventoryGUI* gui)
{
         ...
         it != sections.end(); ++it)
    {
        ...
        if (sMaxTarget == 0)
            sMaxTarget = ((dim == Packer::TARGET_H) ? section->height
                                                     : section->width) - 1;
        if (sMaxTarget > maxTarget)
            maxTarget = sMaxTarget;
    }
    return maxTarget;
}
...
int StackSortUI::PeekAppliedLerSide(InventoryGUI* gui, int target)
{
         ...
         it != sections.end(); ++it)
    {
        ...
        int gridSide = (dim == Packer::TARGET_H) ? section->height
                                                  : section->width;
        ...
        int sourceTarget = 0;
        Packer::Result r;
        if (SortWorker::GetBestResult(section, clamped, r, &sourceTarget)
            && r.allPlaced)
        {
...
void StackSortUI::InitializeUI(InventoryGUI* gui, MyGUI::Widget* arrangeBtn,
                               InventoryGUI* ownerGUI)
{
    // Already initialized for this GUI
    if (FindUIState(gui) >= 0)
        return;
    ...
    DebugLog(std::string("[StackSort] Layout anchor: arrangeBtn (")
             + IntToStr(ac.left) + "," + IntToStr(ac.top)
             + " " + IntToStr(ac.width) + "x" + IntToStr(ac.height) + ")"
             + ", parent (" + IntToStr(pc.left) + "," + IntToStr(pc.top)
             + " " + IntToStr(pc.width) + "x" + IntToStr(pc.height) + ")"
             + (isBodyGUI ? " [body]" : ""));
    ...
    ui.backButton = parent->createWidget<MyGUI::Button>(
        "Kenshi_Button1",
        MyGUI::IntCoord(xBack, startY, arrowBtnW, btnH),
        MyGUI::Align::Default,
        "StackSortBack");
    ...
    ui.sortButton = parent->createWidget<MyGUI::Button>(
        "Kenshi_Button1",
        MyGUI::IntCoord(xSort, startY, sortBtnW, btnH),
        MyGUI::Align::Default,
        "StackSortSort");
    ...
    // Dim toggle
    ui.dimButton = parent->createWidget<MyGUI::Button>(
        "Kenshi_Button1",
        MyGUI::IntCoord(xDim, startY, dimBtnW, btnH),
        MyGUI::Align::Default,
        "StackSortDim");
    ...
    ui.forwardButton = parent->createWidget<MyGUI::Button>(
        "Kenshi_Button1",
        MyGUI::IntCoord(xFwd, startY, arrowBtnW, btnH),
        MyGUI::Align::Default,
        "StackSortForward");
...
{
    UIState& ui = s_uiStates[uiIdx];
    if (ui.stacksConsolidated || ui.currentTarget != 0)
        return;
...
void StackSortUI::CycleTargetForward(int uiIdx)
{
    ...
    if (appliedLerSide > ui.currentTarget && appliedLerSide <= maxTarget)
        ui.currentTarget = appliedLerSide;
...
bool StackSortUI::OnForwardClick(InventoryGUI* gui, MyGUI::Widget* sender)
{
    ...
    if (idx < 0)
        return false;  // not handled — caller should fall through to vanilla
...
void StackSortUI::BuildTopFive(int uiIdx)
{
    ...
    {
        if (validByTarget[t] == eligibleSections)
            candidateT[numCandidates++] = t;
    }
...
void StackSortUI::OnSortRightClick(MyGUI::Widget* sender, int, int,
                                    MyGUI::MouseButton id)
{
    if (id != MyGUI::MouseButton(MyGUI::MouseButton::Right))
        return;

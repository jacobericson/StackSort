...
{
    CRITICAL_SECTION& cs;
    ScopedLock(CRITICAL_SECTION& c) : cs(c)
    {
...
{
    CachedSection()
        : section(NULL), gridW(0), gridH(0), dim(Packer::TARGET_H), itemCount(0), computedUpTo(0), valid(0),
          generation(0), snapshotTick(0), hasOriginalPositions(false), resultLockInit(false)
    {
...
class ResultCache
{
  public:
    ...
    static LONG Insert(InventorySection* section, int gridW, int gridH, Packer::TargetDim dim,
    ...
    static bool IsReusable(InventorySection* section, int itemCount, int gridW, int gridH, Packer::TargetDim dim);
    ...
    static void EvictIfNeeded(InventorySection* const* referencedKeys, int referencedCount, int maxEntries);
